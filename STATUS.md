...
| Substrate confinement | **working, embedded.** `Backend` has two implementations and the tools cannot tell them apart: `Embedded` holds substrate's own `HostDriver` in this process, `Client` reaches a daemon over a socket. The embedded path opens a workspace, writes a file and reads it back on every test run, with no daemon, socket or credential anywhere — `tests/embedded_live.rs`, not ignored. Publication follows the machine either way: three tools without a backend, six with one that confines execution | exec through the embedded driver has not been exercised (this box's delegated cgroup subtree belongs to another deployment); and the socket path is still blocked — see the row below |
