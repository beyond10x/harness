# `b10x-harness` command line, version 2026-08-29.2
...
## What changed since 2026-08-29.1, and what did not

**Strictly additive.** Nothing in `2026-08-29.1` was renamed, removed, or changed in shape: every
flag of `run`, `chat`, `sessions`, `tools`, `events` and `app-server` keeps its spelling, its
`takes_value`, its default, its conflicts and its requirements. A consumer pinned to `.1` is
correct against this binary and needs to change nothing.

What is new is one subcommand and its two verbs:

| added | what it takes |
| --- | --- |
| `workflow` | the word itself, no flags — like any container verb |
| `workflow plan` | `--flow <FILE>` (required), `--max-attempts <N>`, `--json` |
| `workflow run` | everything `run` takes, plus `--flow <FILE>` (required), `--input <TEXT>` (required), `--max-attempts <N>` |

`.1` was cut before this binary had a nested verb, so it recorded only top-level subcommands. This
version records a nested verb under **the words that reach it** — `workflow`, `workflow plan`,
`workflow run` — which is the command line a consumer actually types. That is a wider document
about a wider command line, not a different answer about the old one: every name `.1` recorded is
recorded here with the same rows.
