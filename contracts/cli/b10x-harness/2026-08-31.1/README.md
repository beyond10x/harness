# `b10x-harness` command line, version 2026-08-31.1
...
`context show` and the `toolchains list`, `toolchains show`, and `toolchains validate` inspection
commands arrived additively. Context inspection prints the body-free manifest of the
attributable context package by default, `--json` prints the same manifest as JSON, and `--body`
prints the exact rendered instruction. Its declarations are `--workspace`,
`--instructions-file`, repeatable `--context`, `--no-project-instructions`, and
`--toolchain <auto|NAME[,NAME...]>`. `--toolchain-spec FILE` arrived additively on `run`, `chat`,
`workflow run`, `tools`, and `context show`.

No existing command or flag moved. Existing `--instructions-file` invocations are parsed exactly
as before; their runtime meaning changed from replacement to an operator-authority context layer,
which is a behavior change recorded in the changelog rather than an argv move.
