# `b10x-harness` command line, version 2026-08-31
...
## What changed since `2026-08-30.2`, and what did not

**The argv bytes did not move; workflow dispatch did.** clap accepts exactly the command lines it
accepted under `2026-08-30.2`, so `argv.json` is byte-identical. `workflow run -p <NAME>` now applies
the same profile resolution as `run`, exactly once, and a missing endpoint or model is refused by
name with exit `1` instead of reaching an `expect` panic with exit `101`. That runtime behavior is
why this cut exists even though no parser row moved.
...
| command | flag | field | in `2026-08-30.1` | in `2026-08-31` |
| --- | --- | --- | --- | --- |
