# `b10x-harness` command line, version 2026-09-01
...
## What changed since `2026-08-31.1`

`--execution-path <direct|metaharness>` arrived additively on `run`, `chat`, and `workflow run`.
It defaults to `direct` and tells the model whether this loop talks to its provider itself or is an
inner arm driven and observed by metaharness. Repeatable `--process-write-subtree <DIR>` arrived
additively on those commands and on `tools`; an empty list gives confined processes a read-only
workspace, while each named relative directory is exact write authority enforced by substrate.

No existing command or flag moved. The runtime confinement and context behavior is recorded in the
changelog as well as this additive argv cut.
