# `b10x-harness` command line, version 2026-09-02
...
Repeatable `--mcp-profile <FILE>` arrived additively on `run`, `chat`, `workflow run`, and `tools`.
Each file is a Harness-owned authority document that pins one shared-registry digest and one exact
MCP discovery snapshot, then grants a reviewed subset under local names, prose and envelopes.
`--mcp-registry <FILE>` arrived beside it as an optional override and requires at least one profile;
absent, Harness reads the standalone `b10x-mcp` XDG registry and owner-only OAuth store.

No existing command or flag moved. The outbound MCP behavior is recorded in the changelog as well
as this additive argv cut.
