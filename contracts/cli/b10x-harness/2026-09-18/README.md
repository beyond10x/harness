# `b10x-harness` command line, version 2026-09-18
...
Repeatable `--memory-dir <DIR>` arrived additively on `run`, `chat`, `workflow run`, and `tools`.
Each directory is a vault of `<id>.md` records the caller hands in: YAML frontmatter with `kind`,
`summary`, `trust`, `status` and an optional `supersedes`, the body after, and the id taken from
the file's own name. The summaries go in the standing instruction and the bodies stay behind a
`recall` tool whose `id` argument is a schema `enum` over the active records.

**Absent, a run has no memories and publishes no `recall` tool.** There is no default vault, no
`$XDG` location, no environment variable and no walk up the tree beside the workspace: this flag is
the whole of how a memory reaches a run, which is what distinguishes it from the ambient memory
file a hermetic run refuses. `--plugin-dir` does **not** contribute memories, and no profile key
sets this — a run's vault is named on the command line you can see or it does not exist.

**No memory-writing flag or tool arrived with it, and none is planned as a flag on this one.** The
shipped toolset stays read-only; a writer is its own change with its own gate (`AGENTS.md`).

No existing command or flag moved. The memory behavior is recorded in the changelog as well as
this additive argv cut.

The detailed tables below carry forward the corrections `2026-08-30.2` made to earlier released
documents. They remain here because those earlier directories are immutable and a consumer of the
current pin needs one place that states every historical mismatch.
...
| command | flag | field | in `2026-08-30.1` | in `2026-08-31.1` |
| --- | --- | --- | --- | --- |
...
and it is keyed by command exactly as `arguments` is. Four command paths take positionals:
`profiles show <NAME>`, `providers show <NAME>`, `toolchains show <NAME>`, and the repeated,
required `toolchains validate <FILE>...` input.
...
`1` could not run. They are behavior after clap and are not encoded in `argv.json`.
...
| `workflow run` | `--base-url` | `refused by name` | `always` |
| `workflow run` | `--model` | `refused by name` | `always` |
| `workflow run` | `--session-dir` | `refused by name` | `only where the environment names no state directory` |
...
### Workflow uses the same post-clap rules

`workflow run` flattens the same options and now appears in both tables. Its profile is applied
before walking the flow; a profile may supply endpoint, model, wire, credential and related
options, while an unresolved endpoint or model takes the same named refusal path as `run`.

## Checked from both directions

| half | where |
| --- | --- |
| the manifest digest matches the file | `cargo xtask cli-contract` |
...
| a flag that eats no word names no placeholder | `crates/harness-cli/src/contract.rs`, `a_flag_that_eats_no_word_records_no_placeholder_for_one`, and `cargo xtask cli-contract` |
...
| a positional the binary requires is recorded, and one that becomes required is a move | `crates/harness-cli/src/contract.rs`, `a_positional_the_binary_requires_is_pinned` and `a_positional_that_becomes_required_or_vanishes_is_a_move`, and `cargo xtask cli-contract` |
...
| every rule the checker holds fires on a document that breaks it | `cargo xtask cli-contract --self-test`, its own gate step |
