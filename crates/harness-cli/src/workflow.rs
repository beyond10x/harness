...
//!
//! # A step runs under the write scope its own node declares
//!
//! The one thing about the toolset that is **not** the run's for every step. A node's
//! `run.scope` — the map's own first-match-wins list, in the grammar `--write-scope` takes — is
//! laid over the run's for the length of that step and taken off again however it ends
//! ([`StepRunner::run`], [`harness_tools::Catalogue::narrow`]). It can only narrow: both layers
//! are asked and the first refusal wins, so a document generated by another component cannot give
//! back what the operator's command line took away, and a node declaring nothing runs under the
//! run's exactly as every step did. A `scope` this build cannot read refuses the document by node
//! path in [`payloads`], never falling through to the run's.
//!
...
use harness_flow::{
    Flow, FlowEvent, FlowSink, FlowStatus, Gate, Group, Handoff, Moment, Node, NodeId, Plan,
    Repeat, Report, Step, StepContext, StepOutcome, StepRunner,
};
use harness_loop::{
...
///
/// A `scope` this build cannot read is refused **here**, by name, for the same reason and a
/// sharper one: it is the key that says where the step may write, and a walk that fell through to
/// the run's scope because it could not read the document's would run wider than the document
/// says while every reader of that document believes otherwise.
    ...
    for node in &group.nodes {
        ...
        match node {
            ...
            Node::Step(step) if step.run.is_object() || step.run.is_null() => {
                declared_scope(step).map_err(|reason| format!("`{here}` {reason}"))?;
                action_of(step).map_err(|reason| format!("`{here}` {reason}"))?;
            }
            Node::Step(step) => {
                return Err(format!(
                    "`{here}` carries a `run` payload that is {}: a step's payload is an object of \
                     the keys this runner reads — `kind`, `state`, `summary`, `description`, \
                     `prompt`, `context`, `scope`, `command` — or nothing at all",
                    shape(&step.run)
                ));
            }
        }
...
fn finished(outcome: Result<Report, RunFailure>, json: bool) -> ExitCode {
    match outcome {
        Ok(report)
            if matches!(
                report.status(),
                FlowStatus::Completed | FlowStatus::AwaitingOperator
            ) =>
        {
            ExitCode::SUCCESS
        }
...
/// be a second answer to *what may this run do* (design 0003 § 6 keeps the entries and the
/// programs for M2). The catalogue's **write scope** is the exception: each step narrows it to
/// what its own node declares and gives it back when it ends (module header).
struct FlowRunner<'a> {
...
impl FlowRunner<'_> {
    ...
    fn turn(&self, path: &str, step: &Step, context: &StepContext) -> Result<String, Refused> {
        ...
        // Read from the catalogue and not from the payload a second time: what the tool will
        // enforce is what the step is told, so the two can never come to say different things. The
        // tool is what makes it true; this is what stops the step spending a turn — a paid one —
        // discovering its own document's rule by being refused.
        let narrowed = self.prepared.tools.catalogue().step_scope();
        if !narrowed.is_empty() {
            turn.push_str(
                "\n\nWhere this step may write, on top of the run's own scope. A path no rule \
                 names is unrestricted:\n",
            );
            turn.push_str(&crate::scope_lines(narrowed));
        }
    ...
    /// The step itself, with the catalogue already narrowed to what its node declared.
    fn stepped(&mut self, path: &str, step: &Step, context: &StepContext) -> StepOutcome {
        if self.halted.get() {
        ...
        // The document decides which of the three native step kinds this is. Validation already
        // read the same payload before `prepare`, so an error here would mean the document changed
        // underneath an in-memory walk.
        match action_of(step) {
            Ok(StepAction::Llm) => {}
            Ok(StepAction::Command(argv)) => return self.command(path, index, &argv, budget),
            Ok(StepAction::Operator { prompt }) => {
                return StepOutcome::Paused { reason: prompt };
            }
            Err(reason) => {
                self.warn("step-kind", &format!("`{path}` {reason}"));
                return StepOutcome::Failed;
            }
        }
...
impl StepRunner for FlowRunner<'_> {
    /// One step, under the write scope **its own node declares**.
    ///
    /// The node's `scope` is laid over the run's for the length of the step and taken off again
    /// however the step ends, so the boundary a document states is the boundary the step meets —
    /// in the tool, before the write, as a refused [`ToolOutcome`] the model is told about
    /// (`AGENTS.md` invariants 9 and 12). Until this existed the toolset was built once in
    /// `prepare` and a node's `scope` was decorative: a walk of the eval's map wrote
    /// `revision: 99` into a planning store its own document denied, and only the store's
    /// after-the-fact validator caught it. *Prevented* and *detected* are not the same guarantee.
    ///
    /// **It can only narrow.** Both layers are asked and the first refusal wins
    /// ([`harness_tools::Catalogue::narrow`]), so a node cannot re-permit what `--write-scope`
    /// denied — which is what makes a document generated by another component safe to obey here.
    /// A node declaring nothing runs under the run's scope, exactly as every step did before.
    fn run(&mut self, path: &str, step: &Step, context: &StepContext) -> StepOutcome {
        // An operator step reaches no execution boundary at all. In particular it does not read
        // or apply `scope`, consult a call hook or a budget, open a tool call, or ask a provider.
        // The scheduler records it as reached and stops at the typed pause outcome.
        if let Ok(StepAction::Operator { prompt }) = action_of(step) {
            return StepOutcome::Paused { reason: prompt };
        }
        // Read before anything is narrowed, because a `scope` this build cannot read is a failed
        // step and never a fall-through to the run's. The document was validated at read time, so
        // this refuses only for a document nothing on this path could have produced.
        let declared = match declared_scope(step) {
            Ok(declared) => declared,
            Err(reason) => {
                self.warn("step-write-scope", &format!("`{path}` {reason}"));
                return StepOutcome::Failed;
            }
        };
        self.prepared.tools.catalogue_mut().narrow(declared);
        let outcome = self.stepped(path, step, context);
        // However the step ended. A narrowing that outlived its step would be one node's rule
        // silently governing its siblings, which no document says.
        self.prepared
            .tools
            .catalogue_mut()
            .narrow(harness_tools::Scope::default());
        outcome
    }
...
/// section handed over.
/// The action named by a step's `kind`.
///
/// # Errors
///
...
enum StepAction {
    Llm,
    Command(Vec<String>),
    Operator { prompt: String },
}

/// The native meaning of a step payload.
///
/// Absent `kind` and explicit `llm` preserve the original model-turn behaviour. `command` is one
/// gated tool call. `operator` is a terminal handoff to a person. Anything else is refused before
/// the run is prepared, so a new producer vocabulary can never silently become a paid model turn.
fn action_of(step: &Step) -> Result<StepAction, String> {
    let payload = step.run.as_object();
    let kind = match payload.and_then(|map| map.get("kind")) {
        None => return Ok(StepAction::Llm),
        Some(Value::String(kind)) => kind.as_str(),
        Some(value) => {
            return Err(format!(
                "carries a `kind` that is {}: expected `llm`, `command` or `operator`",
                shape(value)
            ));
        }
    };
    match kind {
        "llm" => return Ok(StepAction::Llm),
        "operator" => {
            let Some(prompt) = payload
                ...
                return Err(
                    "is an `operator` step and carries no non-empty `prompt` for the person"
                        .to_owned(),
                );
            };
            return Ok(StepAction::Operator {
        ...
        other => {
            return Err(format!(
                "declares unknown step kind `{other}`: expected `llm`, `command` or `operator`"
            ));
        }
    }
    ...
    let argv: Option<Vec<String>> = command
        ...
        .collect();
    match argv {
        Some(argv) if !argv.is_empty() => Ok(StepAction::Command(argv)),
...
/// `protocol workflow flow --map` emits into the node — one reader for one grammar, so a
...
/// # Errors
///
/// Names the declaration that could not be read, and what it is instead. A `scope` this build
/// cannot read must never fall through to the run's scope: that is a document stating a boundary
/// and a walk quietly running without it, which is the whole failure this key exists to close.
fn declared_scope(step: &Step) -> Result<harness_tools::Scope, String> {
    let Some(scope) = step.run.as_object().and_then(|map| map.get("scope")) else {
        return Ok(harness_tools::Scope::default());
    };
    if scope.is_null() {
        return Ok(harness_tools::Scope::default());
    }
    let Some(rules) = scope.as_array() else {
        return Err(format!(
            "carries a `scope` that is {}: a step's write scope is a list of \
             `<glob>=<allowed|partial-only|denied>`",
            shape(scope)
        ));
    };
    let declarations: Option<Vec<String>> = rules
        .iter()
        .map(|rule| rule.as_str().map(str::to_owned))
        .collect();
    let Some(declarations) = declarations else {
        return Err(
            "carries a `scope` holding something that is not a string: every rule is one \
             `<glob>=<allowed|partial-only|denied>`"
                .to_owned(),
        );
    };
    crate::write_scope(&declarations)
        .map_err(|reason| format!("carries a `scope` this build cannot read: {reason}"))
}
...
mod tests {
    ...
    /// Each native kind is explicit, and malformed or unknown kinds never fall through to a turn.
    #[test]
    fn step_kinds_are_read_by_name_and_malformed_ones_are_errors_not_turns() {
        ...
        assert_eq!(
            action_of(&step(serde_json::json!({
                "kind": "operator", "prompt": "Approve the specification."
            }))),
            Ok(StepAction::Operator {
                prompt: "Approve the specification.".to_owned()
            })
        );
        assert!(
            action_of(&step(serde_json::json!({"kind": "command"})))
                .is_err_and(|reason| reason.contains("no `command` list")),
        );
        assert!(
            action_of(&step(serde_json::json!({"kind": "command", "command": []})))
        ...
        assert!(
            action_of(&step(
                serde_json::json!({"kind": "operator", "prompt": "  "})
            ))
            .is_err_and(|reason| reason.contains("non-empty `prompt`")),
        );
        assert!(
            action_of(&step(serde_json::json!({"kind": "delegate"})))
                .is_err_and(|reason| reason.contains("unknown step kind `delegate`")),
        );
    }
    ...
    // --- a step runs under the scope its own node declares ---------------------------------

    /// One node, with whatever `run:` payload the case is about.
    fn a_node(run: Value) -> Step {
        Step {
            id: "implement-1".to_owned(),
            needs: Vec::new(),
            run,
        }
    }

    /// The toolset a walk publishes, through the same `published()` path `prepare` uses.
    ///
    /// `unconfined` and not `new` only so that the two writing entries exist to be refused: the
    /// shipped toolset is read-only until a confinement provides one, and a test that stood up a
    /// substrate daemon to watch a rule refuse would be measuring the daemon.
    fn a_toolset(dir: &Path, run: &[&str]) -> crate::Published {
        ...
        .expect("nothing is confined here, so nothing can refuse")
        .tools
    }

    /// The catalogue narrowed to what this node declares, exactly as [`StepRunner::run`] does it.
    fn under(tools: &mut crate::Published, node: &Step) {
        tools
            .catalogue_mut()
            .narrow(declared_scope(node).expect("the node's rules read"));
    }

    /// One whole-file write through the published surface, as a model's call reaches it.
    fn writing(tools: &mut crate::Published, path: &str) -> ToolOutcome {
        use harness_wire::ToolPort as _;
        tools.call(&ToolCall {
            call_id: CallId::new("flow-write-1").expect("a call id"),
            name: ToolName::new("file_write").expect("`file_write` is a tool name"),
            arguments: serde_json::json!({"path": path, "text": "revision: 99\n"}),
        })
    }

    /// A workspace with the store directory the eval's map denies writes to.
    fn a_workspace() -> tempfile::TempDir {
        let dir = tempfile::tempdir().expect("a temporary directory");
        fs::create_dir_all(dir.path().join(".engineering/planning/specification"))
            .expect("the store directory");
        dir
    }

    const SPECIFICATION: &str = ".engineering/planning/specification/passkey-login.md";

    #[test]
    fn a_step_whose_node_denies_a_path_cannot_write_it_and_nothing_reaches_disk() {
        // The eighth paid native walk, in one test. The map's deliberate-denial step denies
        // `.engineering/**` precisely so a refusal can be observed; the run's own scope allows
        // everything, and until the node was read `revision: 99` reached the store on disk and
        // only the store's after-the-fact validator caught it.
        let dir = a_workspace();
        let mut tools = a_toolset(dir.path(), &["**=allowed"]);
        under(
            &mut tools,
            &a_node(serde_json::json!({
                "state": "implement",
                "scope": [".engineering/**=denied", "**=allowed"],
            })),
        );

        let refused = writing(&mut tools, SPECIFICATION);

        assert!(
            refused.failed,
            "the call is refused, and as an outcome the model is told about (invariant 9)"
        );
        let said = plain(&refused.output);
        assert!(said.contains(SPECIFICATION), "{said}");
        assert!(
            !dir.path().join(SPECIFICATION).exists(),
            "refused in the tool layer, before the write: the provider was never reached"
        );
    }

    #[test]
    fn a_node_that_declares_no_scope_runs_under_the_runs_own_and_narrows_nothing() {
        // A document that says nothing does not silently narrow a run. Both halves are the point:
        // what the run denied is still denied, and what it never mentioned is still writable.
        let dir = a_workspace();
        let mut tools = a_toolset(dir.path(), &["target/**=denied"]);
        let silent = a_node(serde_json::json!({"state": "implement", "prompt": "carry on"}));
        assert!(
            declared_scope(&silent)
                .expect("a node with no scope reads")
                .is_empty(),
            "no key is no scope, not an empty one that restricts nothing new"
        );
        under(&mut tools, &silent);

        assert!(
            !writing(&mut tools, SPECIFICATION).failed,
            "the run allowed it"
        );
        assert!(
            writing(&mut tools, "target/debug/x").failed,
            "and the run's own rule still binds"
        );
    }

    #[test]
    fn the_first_rule_a_node_wrote_decides_so_re_ordering_the_list_changes_what_it_means() {
        // First match wins, in the order the map wrote it. The projection emits the list unsorted
        // for exactly this reason, and nothing on this side may sort it either.
        let dir = a_workspace();
        let deny_first = [".engineering/**=denied", "**=allowed"];
        let allow_first = ["**=allowed", ".engineering/**=denied"];

        let mut tools = a_toolset(dir.path(), &["**=allowed"]);
        under(
            &mut tools,
            &a_node(serde_json::json!({"scope": deny_first})),
        );
        assert!(
            writing(&mut tools, SPECIFICATION).failed,
            "the denial is first, so it decides"
        );

        under(
            &mut tools,
            &a_node(serde_json::json!({"scope": allow_first})),
        );
        assert!(
            !writing(&mut tools, SPECIFICATION).failed,
            "the same two rules, the other way round, mean the other thing"
        );
    }

    #[test]
    fn a_node_cannot_give_back_what_the_runs_own_scope_took_away() {
        // A projection is generated by another component and the command line is the operator's
        // own sentence. A node that could re-permit what `--write-scope` denied would make the
        // flag a suggestion, so narrowing is structural: both layers are asked and the first
        // refusal wins.
        let dir = a_workspace();
        let mut tools = a_toolset(dir.path(), &[".engineering/**=denied"]);
        under(
            &mut tools,
            &a_node(serde_json::json!({"scope": [".engineering/**=allowed", "**=allowed"]})),
        );

        let refused = writing(&mut tools, SPECIFICATION);

        assert!(refused.failed, "the run's denial stands");
        assert!(
            !dir.path().join(SPECIFICATION).exists(),
    ...
    #[test]
    fn a_narrowing_lasts_one_step_and_the_next_starts_under_the_runs_scope_again() {
        // What `StepRunner::run` puts back however the step ended. A narrowing that outlived its
        // step would be one node's rule silently governing its siblings, which no document says.
        let dir = a_workspace();
        let mut tools = a_toolset(dir.path(), &["**=allowed"]);
        under(
            &mut tools,
            &a_node(serde_json::json!({"scope": [".engineering/**=denied"]})),
        );
        assert!(writing(&mut tools, SPECIFICATION).failed);
        ...
        assert!(
            !writing(&mut tools, SPECIFICATION).failed,
            "the next step is under the run's scope alone"
        );
        assert_eq!(
            fs::read_to_string(dir.path().join(SPECIFICATION)).expect("written"),
            "revision: 99\n"
        );
    }

    #[test]
    fn a_scope_this_build_cannot_read_is_refused_when_the_document_is_read() {
        // By name, at validate time, and never a fall-through to the run's scope: a document that
        // states a boundary and a walk that quietly ran without it is the failure the key exists
        // to close. `workflow plan` refuses it too, because it validates the same way.
        let refusal = |scope: &str| {
            let flow = Flow::from_yaml(&format!(
                "id: f\nroot:\n  id: root\n  nodes:\n    - id: one\n      run:\n        \
                 state: s\n        scope: {scope}\n"
            ))
            .expect("the document reads");
            payloads(&flow.root, &flow.root.id).expect_err("refused")
        };

        let shape = refusal("\"everything\"");
        assert!(shape.contains("`root.one`"), "{shape}");
        assert!(shape.contains("a string"), "{shape}");

        let word = refusal("[\"crates/**=readonly\"]");
        assert!(word.contains("`root.one`"), "{word}");
        assert!(word.contains("partial-only"), "and the three words: {word}");

        let listed = refusal("[7]");
        assert!(listed.contains("not a string"), "{listed}");
        ...
        .expect("the document reads");
        payloads(&flow.root, &flow.root.id).expect("the grammar `--write-scope` already takes");
    }
}
