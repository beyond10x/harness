//! Bounded server-sent-event reading for the Messages wire.
//!
...
//!
//! # A near-copy of the Responses reader, and what differs
//!
//! The two framings are not the same stream with a different payload. This one has **no `[DONE]`
//! sentinel**: the terminal marker is a `message_stop` *payload*, so end-of-stream here is decided
//! by the decoder above and not by the framer — which is why this reader has no `Done` variant to
//! return and why a `[DONE]` line would be a payload that is not JSON rather than a clean end.
//! The named `event:` field is likewise informational: every payload repeats its own `type`, and
//! trusting the frame name over the payload would let a mislabelled frame decide the turn.
//!
//! The duplication is deliberate and is recorded as a finding rather than hidden: see the crate
//! documentation.
...
impl<R: BufRead> SseReader<R> {
    ...
    /// Reads the next event payload, or `None` at end of stream.
    ///
    /// # Errors
    ///
    ...
    pub fn next_event(&mut self) -> Result<Option<Value>, WireError> {
        ...
        loop {
            ...
            // `event:` names the frame; the payload repeats it as `type` and that is what the
            // decoder reads. Trusting the frame name over the payload would let a mislabelled
            // frame decide the turn.
            if field != "data" {
                continue;
            }
            if saw_data {
    ...
    fn decode(data: &str) -> Result<Value, WireError> {
...
mod tests {
    ...
    fn read_all(body: &str) -> Result<Vec<Value>, WireError> {
    ...
    #[test]
    fn reads_events_and_ignores_comments_and_the_frame_name() {
    ...
    fn a_truncated_event_is_not_a_completion() {
        let error = read_all("data: {\"type\":\"ping\"}").expect_err("truncation refuses");
        assert_eq!(error.code, WireErrorCode::Protocol);
        assert!(
    ...
    fn a_clean_end_of_stream_is_not_an_error() {
        assert_eq!(read_all("").expect("empty stream"), Vec::<Value>::new());
    }
