...
source and the session file — carried the operator's home directory, which
`scripts/check-no-home-paths.py` refuses in a tracked file and which names nobody's machine anybody
needs. In both, the `/home/<account>` head was rewritten to `/home/you`, the placeholder account
that check documents. (Written with `<account>` here for the reason that file writes `<name>`: this
document is tracked, and the check searches it too — the first draft of this paragraph failed the
gate by spelling the thing it was explaining.) No figure, event, token count, digest or ordering
was touched, and the record still parses and still scores identically.
